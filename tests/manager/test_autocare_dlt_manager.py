...
from waffle_hub.hub.manager.adapter.autocare_dlt.autocare_dlt import AutocareDltManager
from waffle_hub.type import TaskType


def _train(manager: AutocareDltManager, dataset: Dataset, image_size: list[int]):
...
def _check_invaild_dataset(
    manager: AutocareDltManager, wrong_task_dataset: Dataset, image_size: list[int]
):
    _delete_artifacts(manager)
    # wrong task dataset
...
def _delete_artifacts(manager: AutocareDltManager):
...
def _test_model_info(task: TaskType):
    ...
    model_type = list(AutocareDltManager.MODEL_TYPES[task].keys())[0]
    model_size = list(AutocareDltManager.MODEL_TYPES[task][model_type].keys())[0]

    return model_type, model_size


def test_cls_autocare_dlt_manager(
    ...
    temp_dir = tmpdir / f"{AutocareDltManager.BACKEND_NAME}_{task}_{model_type}_{model_size}"
    manager = AutocareDltManager(
        root_dir=temp_dir,
        name="test",
        task=task,
...
def test_od_autocare_dlt_manager(
    classification_dataset: Dataset, object_detection_dataset: Dataset, tmpdir: Path
):
...
def test_sem_seg_autocare_dlt_manager(
