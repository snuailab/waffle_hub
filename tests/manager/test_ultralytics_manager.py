...
def _train(manager: UltralyticsManager, dataset: Dataset, image_size: list[int]):
...
def _check_invaild_dataset(
    manager: UltralyticsManager, wrong_task_dataset: Dataset, image_size: list[int]
):
    _delete_artifacts(manager)
    # wrong task dataset
...
def _delete_artifacts(manager: UltralyticsManager):
...
def _test_model_info(task: TaskType):
    ...
    model_type = list(UltralyticsManager.MODEL_TYPES[task].keys())[0]
    model_size = list(UltralyticsManager.MODEL_TYPES[task][model_type].keys())[0]

    return model_type, model_size


def test_cls_ultralytics_manager(
    ...
    temp_dir = tmpdir / f"{UltralyticsManager.BACKEND_NAME}_{task}_{model_type}_{model_size}"
...
def test_od_ultralytics_manager(
    classification_dataset: Dataset, object_detection_dataset: Dataset, tmpdir: Path
):
...
def test_ins_seg_ultralytics_manager(
